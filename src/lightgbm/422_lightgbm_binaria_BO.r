...
hs <- makeParamSet(
  ...
  makeNumericParam("feature_fraction", lower = 0.1, upper = 1.0),  # También conocido como colsample_bytree
  makeNumericParam("bagging_fraction", lower = 0.1, upper = 1.0),  # Muestreo de las filas (como sub_sample)
  makeIntegerParam("min_data_in_leaf", lower = 1L, upper = 8000L),
  makeNumericParam("min_sum_hessian_in_leaf", lower = 1e-5, upper = 10),  # Regularización sobre hojas
  ...
  makeIntegerParam("max_depth", lower = -1L, upper = 16L),  # Profundidad del árbol, -1 para ilimitado
  makeIntegerParam("n_estimators", lower = 100L, upper = 10000L),  # Número de árboles (antes envios)
  makeIntegerParam("bagging_freq", lower = 1L, upper = 10L),  # Frecuencia de bagging (entero)
  makeNumericParam("min_gain_to_split", lower = 0.0, upper = 15.0)  # Ganancia mínima para hacer un split
)
